...
def main():
    ...
    if a.remote:
        ...
        if auth.returncode != 0:
            say("   gh is present but NOT authenticated, and authenticating is")
            say("   yours to do: it opens a browser and issues a token, and I")
            say("   neither type nor see credentials. Three routes, pick one:")
            say()
            say("   A) Authenticate gh, then re-run GO.bat. NOTE the full path --")
            say("      this gh is bundled inside Copilot Desktop and is NOT on")
            say("      PATH, so the bare command will not resolve:")
            say('          "%s" auth login' % gh)
            say("      choose GitHub.com -> HTTPS -> login with a web browser.")
            say()
            gd = find_github_desktop()
            if gd:
                say("   B) GitHub Desktop, four clicks and no typing:")
                say("        File -> Add local repository -> %s" % HERE)
                say("        Publish repository")
                say('        LEAVE "Keep this code private" TICKED -> Publish')
                say()
            say("   C) Create an empty repo at https://github.com/new (private),")
            say("      then let Git Credential Manager authenticate the push:")
