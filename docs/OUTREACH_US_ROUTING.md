...
- **Event A (§5, the only one that changes anything):** someone independent computes all 23 vectors from their `input` fields in `docs/CONFORMANCE_SPEC.json`, in a language that is not Python, sharing no source, and every `expected` matches. **Corrected 2026-09-15:** this used to read "reproduces the conformance root `0c398099…0f0ddcef`", which is not an achievement — the root hashes the expecteds printed in that same file and falls out of it in nine lines. An outside reviewer did exactly that to refute the claim (A121).
...
And the flat truth underneath all of it, which §8 states already and which this map only confirms: **the letters are not the bottleneck.** Nothing that could be written this week changes any of these outcomes. What changes them is one licence file, one real entry in `peers.txt`, and one stranger computing all 23 vectors from their inputs and matching every answer (NOT merely reproducing `0c398099…0f0ddcef`, which is free — A121). Two of those three are code, and the third is what the two sends above are actually for.