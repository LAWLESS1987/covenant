...
| **P17** | **CLOSED** 09-12 | a restart could take the chain down and LEAVE it down — `AB_RESTART_NODES.bat` stopped the nodes, then `covenant_prod.bat` refused to start them when nothing answered on 11434. Both probes were retired with the server; a start is never blocked by a missing server, and the gate itself still fails closed by design. Gated in `verify_deploy.py`; the bare double-click is still exposed |
...
| P15 | **CLOSED** 09-07 | the fourth long-lived process it wanted identified — the local model server — was removed from this PC; the seat is the distilled student, in-process |
...
### 2026-08-27 ~02:45 → ~03:10 — (ATTENDED, L: "improve for mutual benefit") the suites that were never run, and the console that answered for a process nobody started

**Selection.** Nothing was chosen from the backlog. The three claims were
measured first and came back clean — `verify_bundle.py` 301/301, disk v8.37
`07e097f3e37f`, and by the end of the run G9 had all three nodes reporting
v8.37 == disk — so the drift question that usually opens a run had no work in
it. P17 turned out to be CLOSED already (the guard is in `AB_RESTART_NODES.bat`
at line 24, dated 08-26); the "Next" list in the entry above predates it. What
was picked instead came from a question no run had asked: **which test files on
disk does either runner actually call?**

**Ten did not. That is the finding, and it has two halves that point opposite
ways.** Diffing `test_*.py` on disk against the two suite lists: four suites
ship in `MANIFEST.sha256`, are hashed on every launch, and run in NEITHER
runner — and eleven suites are INVOKED by `run_all_tests.sh` and are not on
disk. `run()` there scraped a tally out of stdout with stderr sent to
`/dev/null`, so an absent file produced no tally, contributed `0 passed, 0
failed`, printed `NO RESULT`, and left `FAILED` untouched. **Eleven named
checks could be missing and the sweep still ended green**, and the README's
"33 suites, 1,043 checks" is counted from that runner. A suite nobody runs and
a suite that is not there both read as coverage in a file count. Neither is.

**Measured standalone on win32 before wiring anything, because a suite that
does not pass turns the sweep red for a reason that is not the node's:**
`test_r1_lora_frame` 58/58, `test_backtest_guardrails` 16/16,
`test_3node_config` 11/11 — **85 checks that had never run on this machine** —
and `test_covenant_app` **which did not pass. It died on a KeyError.** That is
the run.

**`test_covenant_app.py` was measuring a console nobody had started.**
`state()["ethics_review"]` raised `KeyError` against a field
`covenant_app.py` demonstrably emits. The reason: a console from 20:18 the
previous evening — `48fff56ce81c`, 530 lines, built before the field existed —
was still listening on 5199, the suite's port. The suite has a busy-port
preflight and it reported the port FREE. `start_app` then bound a SECOND socket
on 5199, `netstat` showed **two processes LISTENING on the same address**, and
`/api/identity` was answered by the old one. H17c read a build from the night
before and reported it as a defect in today's code.

**This is A19, six days later, in the file that shipped after the fix.** The
node solved it in v8.30: `_bind_exclusive`, whose docstring says in as many
words that on Windows "its probe bind succeeded while a node was listening, so
the check passed on the exact platform where the footgun is worst".
`covenant_app.py` uses stock `ThreadingHTTPServer`, which sets
`allow_reuse_address = 1`; the suite's preflight set `SO_REUSEADDR` on purpose,
with a comment reasoning that it should ask the same question the server will
ask. **On POSIX that reasoning is correct. On Windows it is inverted**, and the
comment is what made it survive four days of reading.

**The TIME_WAIT worry that chose `SO_REUSEADDR` was the only cost of changing
it, so it was measured rather than argued about** — a controlled bind on a
scratch port, own server closed with a TIME_WAIT on it: EXCLUSIVE binds, REUSE
binds. **It costs nothing.** With a LIVE listener: EXCLUSIVE refuses 10048,
REUSE binds. The suite loses nothing and gains the collision.

**And the first version of that fix was still wrong, which is the part worth
keeping.** Patched to `SO_EXCLUSIVEADDRUSE`, the preflight caught 5199 and
still called 5000/5020/5060 free **while nodes A, B and C were listening on
them**. A node binds `0.0.0.0`; the console and the stubs bind `127.0.0.1`; a
probe only sees a listener its own bind overlaps. Measured with both kinds of
listener live, BOUND meaning the probe MISSED a running process:

```
  probe bind    opt         :5000 (node)  :5199 (console)
  0.0.0.0       exclusive   REFUSED       REFUSED      <-- the only row
...
**The option and the address are one answer, not two**, and getting the option
right had made the check look fixed while half of it still lied.

**The deeper repair is in the harness, not the socket.** A reply on the port
was the whole readiness test, which is only sound if nothing else can hold the
port. `start_app` now compares the answering console's own `sha` against
`covenant_app.py` on disk and stops with UNKNOWN if a stranger answers — M38
turned on the thing doing the measuring rather than on the node. The console
also got `ExclusiveHTTPServer`, so a second one now refuses to start on a taken
port using the error message it already had and had never been able to print.

**Verification.** With the chain up the suite now says so and stops —
`ports [5000, 5020, 5060, 5199] are already in use ... UNKNOWN, not a pass`
(exit 2) — where four days ago it would have run. On a shifted port block with
everything staged: **54/54, twice.** The three wired suites through
`run_local_sweep.py`: 58, 16 and 11, rc=0 each, with `test_3node_config`
finding the `.bat` files only because `stage()` now copies them.

**Not wired, deliberately, and this is a real residual.**
`test_covenant_app.py` (54 checks) stays out of both runners. It binds the
production ports because the console reads its `NODES` table from source and
there is no env var that moves it — by design, and the same design that keeps
`BIND_HOST` a literal. So it can only run with the chain STOPPED while the
sweep is meant to run with it up, and wired it would report UNKNOWN on every
normal sweep. **A check that is structurally always UNKNOWN teaches people to
skip the line it prints**, which is how this run's four orphans got to be
orphans. Switched off honestly, with the reason written at the point of
omission so the next disk-versus-runner diff finds an answer instead of a gap.
**Moving it in means giving `covenant_app.py` an overridable NODES table. That
is a change to a security-sensitive file and it is L's call, not a wiring
decision.**

**Two smaller things, both the same shape.** `test_a12_dead_peers.py` was the
PRE-fix A12 suite, left behind by the rename to
`test_a12_dead_peer_backoff.py`; it asserts the ABSENCE of the backoff, scores
4/24 against the node on this disk, and kept a `test_*.py` name that invites
exactly the diff this run did. Renamed to `test_a12_dead_peers.PRE-A12FIX.py`
with the reason on top, for the same reason INDEX.md renamed the stale HBAR
series. And `sim1000_invariants.py` hardcoded `/tmp`, so on the machine that
runs this project it could only ever raise `IndexError` from a bare `[-1]`; it
now reads `tempfile.gettempdir()`, matches directories rather than any file
sharing the prefix (its first Windows run picked a `.bak` file as the "run"),
and reports **UNKNOWN rather than "FAIL: consensus divergence" when there is
nothing to compare** — it was printing a consensus break over an empty
directory and then dying two lines later.

**Delivered, all in place on L's machine, `verify_bundle.py` 301/301 after:**
`covenant_app.py`, `test_covenant_app.py`, `run_local_sweep.py`,
`run_all_tests.sh`, `sim1000_invariants.py`, `verify_deploy.py` (both runner
hashes repinned in the same edit, as the entry above required),
`MANIFEST.sha256` rewritten, and the rename. **No node source was touched** —
`covenant_unified_v8.py` is still `07e097f3e37f`. Gates after: **G1–G11 PASS**,
G9 all three nodes v8.37 == disk, G12 UNKNOWN pending the sweep that was
started at the end of this run.

**Pushback against my own framing, recorded.** The first read of the "Next"
list put P17 and the sweep at the top; both were stale — P17 was closed the day
before and the sweep only needed running. The item worth doing was not on any
list, because every list here is written in terms of the node, and the thing
that was broken was the apparatus that decides whether the node is broken.
**Three of this run's five findings are in the measuring instruments**, which
is the same lesson as the entry above it from the other side: *write the check
that could embarrass you, then read the failure instead of adjusting the
check.* The console suite's comment was a four-day-old argument for why the
wrong option was the right one.

**Cost:** ~⅓ session. Most of it on the two port measurements; the edits took
minutes.

**Next.** The sweep result (G12) is the immediate one. Then L's calls, both
new: **the `covenant_app.py` NODES table** (above), and **the stale console on
5199** — PID 13456, running `48fff56ce81c` from 20:18 on 08-26, still serving a
page with no ethics-review panel. It was left alone rather than killed, because
a running console is L's. The eleven absent suites in `run_all_tests.sh` will
now go red on the next Linux run; whether they are gone for good or live in the
project and never reached the machine is a **DIVERGENCE_REPORT** question and
is not answerable from here.

**ADDENDUM ~03:20 — the sweep this run started closed G12, and A23 got its first Windows measurement. 36 suites, 10.4 min, and the four reds are three old ones and one that had never been asked.**

Archived as `docs/results/SWEEP_RESULTS_2026-08-27_win32.txt`. **The three
suites wired this run were green through the runner** — r1_lora 58, backtest
guardrails 16, 3node_config 11, rc=0 each — and `test_3node_config` found
`covenant_prod.bat` and `AB_RESTART_NODES.bat` only because `stage()` now
copies them, which is the change proving itself on its first real use.

**Compared line by line against `SWEEP_RESULTS_2026-08-24_win32.txt` rather
than judged on its own**, which is the only way to tell a new red from an old
one:

- `test_security_audit.py` 128/1 — **identical to 08-26.** The one red is
  `identity key file is owner-only`, which is **P9**: the POSIX mode bit that
  NTFS does not honour. G8 passes in the same minute by reading the ACL
  instead. Permanently red on this platform, documented, and switched off in
  the only honest sense — L's approval still outstanding on the code change.
- `test_a1_kill_matrix.py` 28/30 — **identical to 08-26.** Pre-existing.
- `probe_final_pass.py` rc=1 — `xrpl-py not importable`. G3 reports xrpl as an
  absent OPTIONAL import in the same run; the XRP path is shut and this is the
  gate saying so.
- **`test_a23_ack_health.py` 22/24 — and this one is NOT in the 08-26 archive
  at all**, because A23 was not on this machine until the evening of 08-26.
  This is **A23's first execution on Windows**, which is exactly what the entry
  above listed as waiting on the Windows sweep. **22/24 twice, alone**, so it
  is a stable result and not a load artifact.

**Both A23 reds are the same S3 check, and the suite states its own
disqualification in the failure text:** `a REFUSED peer costs almost nothing by
comparison  1.056 s vs 1.07 s`, then `measured ~1x at a 0.3s timeout; at the
5 s default it is 15.1 s vs ~0.13 s (~114x)`. The ratio S3 asserts holds at the
production timeout and collapses at the compressed one the test uses to stay
fast — on Windows a refused connect does not return as cheaply as it does on
Linux, so the compression that is invisible there is the whole measurement
here. **M29 in its own right: this is a claim about a platform, and the suite
made it on the wrong one.** It is a test-calibration finding, not a node
defect — nothing in `_send_raw` is implicated — but S3 as written cannot pass
on win32 and should either take the timeout from the same constant it is
reasoning about or say plainly that it is Linux-only. Not fixed here: it is a
change to an adversarial check, and this run had already touched enough of the
measuring apparatus. **Filed for L.**

**No suite regressed.** Nothing in this run's diff touches node source, and the
only staging change adds files.

`SWEEP_RESULTS.txt` in the folder was then overwritten twice by targeted
re-runs (`--repeat 2 test_a23_ack_health`, then `test_security_audit` alone) to
get the two results above, which left G12's freshness file describing a
one-suite run. A second full sweep was started rather than leaving that in
place, and rather than hand-writing a file a tool is supposed to produce.

**ADDENDUM 2 ~03:35 — the second sweep is byte-for-byte the same verdict, and getting G12 to notice it turned up one more blind gate.**

**36 suites, 12.1 min, the same four reds with the same tallies** (a23 22/24,
security_audit 128/1, kill_matrix 28/30, probe_final_pass on absent xrpl) and
the three wired suites green again. **2/2 on the full sweep**, so this run's
changes are confirmed rather than observed once. `SWEEP_RESULTS.txt` is now a
genuine full run.

**And G12 could not see the archive this run had just written.** The win32
slot was a HARDCODED path — `docs/results/SWEEP_RESULTS_2026-08-24_win32.txt` —
so it reported a date three days stale while a fresher result sat in the same
directory, and would have kept doing so for every future sweep. Now it globs
`SWEEP_RESULTS_*_win32.txt` and takes the newest, still printing the filename
so the platform claim stays visible. **That is the fifth instance today of the
same shape**: a check that cannot see the thing it exists to check. Four of
this run's six findings are in the measuring apparatus, none in the node.

Final state: **G1–G11 PASS**, G12 UNKNOWN by construction and now naming
today's archive, `verify_bundle.py` **302/302**, `covenant_unified_v8.py`
untouched at `07e097f3e37f`.

**ADDENDUM 3 ~03:40 → ~04:12 — L: "killed it update and run". The restart path had three defects stacked on each other, and the top one made the other two invisible.**

L killed the stale console. The machine came back with **nothing running at all** —
three nodes, watchdog and console all down, watchdog's last line 03:37 — which
made this the first moment `test_covenant_app.py` could run where it is meant
to. **54/54 in place, on the real production ports, first time ever on this
machine.** The suite's own A3 check ran `test_3node_config.py` out of the real
folder and got rc=0, where the scratch copy had got rc=1 for want of the `.bat`
files. Then the restart, and the restart is the entry.

**Defect 1 — `call covenant_prod.bat` cannot be resolved by a parent that
switches off cmd's current-directory search.** `AB_RESTART_NODES.bat` line 20
does `cd /d "%~dp0"`, so the folder is right; cmd still refused with
`'covenant_prod.bat' is not recognized as an internal or external command`.
Cause measured rather than guessed: **`NoDefaultCurrentDirectoryInExePath=1`**
in the process environment — Git Bash exports it, so does anything launched
from it, and so does the documented `python verify_deploy.py` path. Reproduced
both ways in one command: with the variable set, cmd refuses; cleared, it
resolves. **The stop above it always succeeds**, so under that parent this
script stops the nodes and cannot start them — **the exact take-it-down-and-
leave-it-down outcome P17 exists to prevent, arriving through a door P17 does
not watch**, since P17 asks about the judge and this is the launcher not
resolving. Fixed to `call "%~dp0covenant_prod.bat"` here and in
`AE_KILL_STRAY_START_B.bat`, the only other bare `.bat` call in the folder.

**Defect 2 — `verify_deploy.py` could not survive a restart that WORKED.**
With defect 1 fixed the launcher started three node windows and a watchdog, and
the verifier hung. `capture_output=True` hands the grandchildren the captured
stdout handle; they outlive the `cmd` that spawned them, so the pipe never
reaches EOF, and `subprocess.run`'s timeout kills only the direct child before
calling `communicate()` again — which blocks with **no deadline at all**.
Measured: **12 minutes past a 300 s timeout**, all three nodes up and answering
`/health`, the verifier still waiting on a pipe held open by the very processes
it was about to verify. **It had never shown up because the only previous run
of this step hit defect 1 and started nothing**, so there were no grandchildren
to hold the pipe: a FAILED restart returned cleanly and a SUCCESSFUL one hung.
The verifier had never once completed the thing it exists to verify. The
launcher writes its own transcript, so the pipe is now `DEVNULL`.

**Defect 3 — and this is the one that would have been believed.** With the pipe
gone the launcher returned **255**, and `verify_deploy` printed `exit code 255`
and then reported **`RESULT: PASS -- project, disk and running process all
agree.`** Every word of that was true. The conclusion any reader draws from it
— that the restart worked — was false: step 4 had found the PREVIOUS restart's
nodes, still up and still matching disk. **Step 4 answers "is the right version
running", never "did MY restart put it there".** Only the launcher's exit code
speaks to that, and nothing read it. A non-zero launcher is now a FAILURE that
says in as many words that what follows is the state that was already running.

**Why the 255, which is defect 1's mirror image.** `AB_RESTART_NODES.bat` ran
`call covenant_prod.bat >> "%OUT%" 2>&1` — so **every node it starts inherits a
handle on `NODE_RESTART.txt`, this script's own report, and holds it for as
long as the node runs.** The next restart then cannot write a single line of
its own transcript: every redirect answered `The process cannot access the file
because it is being used by another process`, the script stopped nothing,
started nothing, wrote nothing and returned 255. Proven rather than inferred —
a control write to any other filename in the same folder succeeded in the same
second, so it was that one file and not the folder, and the four `cmd.exe`
children all carried the launcher's PID as their parent. **The restart path
could run exactly once per boot of the chain.** The launcher now writes to
`%TEMP%\covenant_prod_%RANDOM%%RANDOM%.txt`; the handle still leaks to the
nodes, it just leaks onto something disposable.

**Verification, and it had to be the back-to-back case.** From a stopped chain:
**PASS**, exit 0. Then immediately again, with restart 1's nodes still up —
the case that returned 255 an hour earlier — **PASS**, exit 0, and the node
PIDs changed (15604/18284/4804 → 13468/17080/29212), so the processes were
genuinely replaced rather than re-found. `NODE_RESTART.txt` rewritten, its stop
section listing no surviving ports. And before the fix, the same second run
correctly reported **FAIL — restart launcher exited 255**, which is the check
proving itself in the failing direction as well as the passing one.

**One thing found and NOT fixed, because it is a judgement about the stop
policy.** `covenant_prod.bat stop` kills by `taskkill /fi "windowtitle eq
Covenant Node A*"`. Started headlessly the nodes have no such window title, so
`stop` reported `stopped. Databases untouched` and **all three nodes were still
listening**. `AB_RESTART_NODES.bat` survives this because it has a stop-by-port
fallback; `covenant_prod.bat stop` on its own does not, and it says it
succeeded. Filed for L: a stop that cannot see what it did not start should say
UNKNOWN, not `Stopped. Nothing was deleted.`

**State at the end.** Chain **up and converged** — A/B/C all v8.37
`07e097f3e37f` 9846 lines == disk, heights [3, 3, 3], one shared genesis
`00009b31c6c6` on all three (`own_genesis` true only on A, the founder that
minted it), watchdog alive inside its 60 s bound. Console restarted on the
CURRENT build, `a5eb0780e00a` 607 lines, **actions OFF**, loopback, and its
`/api/state` now carries `ethics_review` — present as `null`, which is the
honest answer until a node's `/health` starts reporting the field in v8.38.
The remaining `/health` warnings are the documented win32 ones: no fork-based
sandbox, and B2's single semantic judge.
