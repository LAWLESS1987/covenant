...
| GitHub Actions, private artifact | `science.yml` since 2026-09-26, when the artifact became science only: it runs the four checks its README lists (every judge figure recomputed from data, the statistics, retractions, the manifest) on Python 3.9 and 3.12. The old `covenant.yml` sweep went with the full system into the archive the operator holds (commit `0097c26`, A228) | on push and pull request | github.com/LAWLESS1987/covenant-satc/actions |
