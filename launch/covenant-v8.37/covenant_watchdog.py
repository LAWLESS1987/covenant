...
def one_pass(strict=False):
    ...
    if all(os.path.exists(os.path.join(HERE, n["db"])) for n in NODES) and \
        ...
        for who, keyfile in (("founder", "nodeA_prod.db.key"),
                             ("nodeB", "nodeB_prod.db.key")):
            if not os.path.exists(os.path.join(HERE, keyfile)):
                continue
            a = balance("nodeA_prod.db", keyfile)
            b = balance("nodeB_prod.db", keyfile)
            if a is None or b is None:
                log("INFO", f"balance read for {who} unavailable "
                            f"(A={a} B={b}) -- skipped, not an alert")
                continue
            if abs(a - b) > 1e-9:
                alerts.append(
                    f"FORK: {who} balance disagrees across databases -- "
                    f"nodeA_prod.db says {a}, nodeB_prod.db says {b}. Tip "
                    f"equality is not state equality; do not transact.")
            else:
                log("INFO", f"{who} balance agrees across both dbs: {a}")
