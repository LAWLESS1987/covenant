...
def g7():
    ...
    if not live:
        return R("G7", g7._title, PASS,
                 "all six ports free -- nothing is running, a cold start is clean.")
    return R("G7", g7._title, PASS,
             "in use (expected if the nodes are up): " +
             "; ".join("%s %s" % (k, v) for k, v in live.items()))
...
def g9():
    ...
    for nid, port in NODES:
        h, err = http_json("http://127.0.0.1:%d/health" % port)
        if h is None:
            unknown.append(nid)
...
def g10():
    ...
    if age > 300:
        return R("G10", g10._title, BLOCKED,
                 "last line was %d min ago, against a ~60 second bound. The "
                 "watchdog process is DEAD. Its final line still asserts that "
                 "everything is healthy, and will go on asserting it: %r"
                 % (age / 60, last),
...
def g11():
    ...
    if not have_proof:
        return R("G11", g11._title, BLOCKED,
                 "; ".join(bits) + ". The XRP submission path has never "
                 "executed on any network. This gate is in code, not advice.",
                 "python test_xrp_live.py   on a networked machine. It funds a "
                 "testnet account, submits one real payment, and writes the "
                 "proof. Nothing here can or should do that for you.")
    return R("G11", g11._title, UNKNOWN,
             "; ".join(bits) + ". A proof file exists; whether the policy names "
             "an address you have checked against its source is not something "
...
def g12():
    cands = [("SWEEP_RESULTS.txt", "local sweep"),
             ("docs/results/SWEEP_RESULTS_2026-08-24_win32.txt", "win32 sweep")]
    found = []
