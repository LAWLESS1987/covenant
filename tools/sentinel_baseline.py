...
# FROZEN 2026-09-18. Twelve files.
#
# RE-FROZEN TWICE the same day, and each time this tool is what reported the
# drift as UNACCOUNTED FOR -- which is what it is for. The accounting is the
# commit that moved the bytes:
#
#   original (commit fa13845)   100,369 bytes   gate suite 28/28
#   + abstention, witness sep   113,538         40/40   (seal_service, tradeGate,
#                                                        test_sentinel_gate)
#   + the judge attack (X1)     118,665         41/41   (seal_service, and S2
...
FROZEN = {
    ...
    "sentinel_witness/seal_service.py":         (15359, "7f9171beb99b3ae7248148557773b3a999aba883bf65c2d0ab9328956020a556"),
    ...
    "test_sentinel_gate.py":                    (26896, "c877e9225b819dcfdf9c4b1b1b134e6f7d965f1a8a42431a62ef02bf06be7259"),
...
def check(say=print):
    ...
    for k, v in sorted(FROZEN_TALLIES.items()):
        say("    %-28s %s" % (k, v))
    say("")
    # NO HARDCODED COVERAGE CLAIM LIVES HERE ANY MORE, and that is a decision
    # rather than an omission. This block twice printed a fact that had rotted:
    # first "there is NO ABSTENTION STATE in this path" after AB1-AB9 added one,
    # then "NOTHING HERE ATTACKS THE JUDGE" after test_x1_judge_adversarial did.
    # Both were true when written, hours apart. A tool that prints a claim about
    # the system is making a claim, and it decays exactly like prose in a
    # document -- so updating it a third time would have been treating the
    # symptom. What is measured is printed; what needs measuring is pointed at.
    say("  WHAT THESE TALLIES DO NOT COVER is not asserted here -- a hardcoded")
    say("  answer rotted twice. Ask the things that measure it:")
    say("    python tools/spec_conformance.py     which requirements are verified,")
    say("                                         and by which checks")
    say("    python test_x1_judge_adversarial.py  the judge's own susceptibility,")
    say("                                         measured and not asserted")
    return ok, rows
