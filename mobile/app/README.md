...
- **From GitHub (private for now, 2026-09-12):** every push to `main` that
  touches the app or the node's files rebuilds it and proves it on the
  emulator, but publishes NO release -- the eight public releases were taken
  down. The APK is the workflow run's artifact `covenant-node-apk` (Actions
  tab, 14 days, a GitHub login required), or build it yourself below. The
  source stays here; moving it to a private repository is a separate step. Samsung: Settings > Security and privacy >
