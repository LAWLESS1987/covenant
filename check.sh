...
if [ -n "$PY" ] && [ -f conformance.py ]; then
    ...
    if [ $RC -eq 0 ] && [ -n "$ROOT" ]; then
        ...
        say "      $ROOT"
        say "      THE ROOT ITSELF IS NOT EVIDENCE OF A REIMPLEMENTATION."
        say "      Jens Egholm Pedersen (DTU), whose NIR idea this borrowed,"
        say "      reproduced it on 2026-09-15 in a few lines straight from"
        say "      docs/CONFORMANCE_SPEC.json without implementing climb or"
        say "      attest at all -- the expecteds are printed in the same"
        say "      file that publishes the root. His verdict: this is an"
        say "      ordinary test-vector suite, not NIR's move, because the"
        say "      reference is an oracle of this code's own outputs rather"
        say "      than a specification of the semantics. See KNOWN_ISSUES"
        say "      A121. The claim stands corrected, not deleted."
        say "      WHAT THE EVIDENCE ACTUALLY IS: two builds sharing no code"
        say "      with this tree computed each vector and matched expected"
        say "      PER VECTOR on 2026-09-03 -- conformance_indep/*.ps1 and"
        say "      *.py, each written from the spec file alone by an AI agent"
        say "      forbidden to read this tree, audited for that, and rerun"
        say "      by test_n2_independent_root.py on every sweep. A suite"
        say "      pins the computation only where it samples: two earlier"
        say "      builds matched the PREVIOUS 11 vectors on 2026-08-31 and"
        say "      five WRONG readings passed too, so twelve were added."
        say "      That is the limit of the method, not a bug it outgrew."
        say "      Nobody outside this project has run the vectors yet."
        PASS=$((PASS + 1))
    else
...
say "  can do -- clone it somewhere this machine cannot reach and run it"
say "  again. NOT to see whether the roots match: a root matches by being"
say "  copied forward through a hash, which is how the biggest claim in"
say "  this tree got refuted on 2026-09-15 (KNOWN_ISSUES A121). The ask is"
say "  to compute the 23 vectors from their INPUTS, in any language, and"
say "  see whether each answer matches. Disagreement is the useful result."
say ""
exit 0
