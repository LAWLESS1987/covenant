...
if [ -n "$PY" ] && [ -f conformance.py ]; then
    ...
    if [ $RC -eq 0 ] && [ -n "$ROOT" ]; then
        ...
        say "      $ROOT"
        say "      REPRODUCED BY TWO BUILDS THAT SHARE NO CODE WITH THIS TREE"
        ...
        say "      Nobody OUTSIDE this project has reproduced this root yet."
        say "      Anyone can, in any language, sharing none of this code,"
        say "      and without asking permission."
        PASS=$((PASS + 1))
    else
